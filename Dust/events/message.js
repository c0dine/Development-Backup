...
module.exports = class {
  ...
  async run (message) {
    ...
    // Dm me the content of the message sent to the bot 
    if (!message.guild) {
      this.client.dmLog(message, this.client.appInfo)
    }
    ...
    // well add embeds to be available from client.embed
    const Discord = require('discord.js')
    message.embed = new Discord.MessageEmbed().setColor('#03fcf0')
    ...
    //make the bot start typing signifying that it Got a command
...
This command requires level ${this.client.levelCache[cmd.conf.permLevel]} (${cmd.conf.permLevel})`);
    ...
    this.client.ownDel(message, defaultSettings)
  };
};
