...
import _thread
# HOST = "127.0.0.0" 
HOST = socket.gethostbyname("")  #--- this also works but idk if its bad maybe uncomment it if stuff doesn't work
PORT = 11909
#rlog_port = 11909
...
def on_new_client(connection, client_address):
    ...
    try:
        ...
        while True:
            ...
            colors=["\x1b[34m", "\x1b[36m", "\x1b[32m", "\x1b[35m", "\x1b[31m", "\x1b[37m", "\x1b[33m"]
            for x in stringarray:
                print(colors[0], end = '')
                print('%s' % x)
                i+=1
                colors.sort(key=colors[0].__eq__)
